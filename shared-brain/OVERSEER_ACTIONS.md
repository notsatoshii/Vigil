...
ACTION|CRITICAL|build|Fix scheduler-state.json stage tracking: completed pipeline stages (plan/critique/build/verify) must update the task stage field so the scheduler can auto-advance tasks instead of leaving everything stuck at "backlog"
